...
class TestKimSL:
    ...
    def test_random_noise_effect(self, noise_std):
        ...
        sl_clean = KimSL(version="wrist").calculate(data_clean, initial_contacts, sampling_rate_hz=100)

        rng = np.random.default_rng(0)
        data_noisy = data_clean + rng.normal(scale=noise_std, size=data_clean.shape)
        sl_noisy = KimSL(version="wrist").calculate(data_noisy, initial_contacts, sampling_rate_hz=100)
    ...
    def test_all_nan_with_zero_ic(self):
        ...
        sl = KimSL(version="wrist").calculate(data, initial_contacts, sampling_rate_hz=100)
